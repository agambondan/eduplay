...
const WORD_LIST = [
  'bunga','cinta','dunia','gajah','hakim','jalan','kanan','lawan','makan','nalar',
  'pagar','rakan','salam','taman','udara','wajah','yakin','zaman','abadi','bakat',
  'calon','damai','empat','fakta','galak','harga','ilham','janji','kabar','layar',
  'mahal','nakal','obral','paham','rapat','sabar','tanah','ujung','viral','waktu',
  'akhir','bahan','capai','dafar','etika','fatwa','gagal','halal','ihsan','jarak',
  'kabel','lapar','mandi','nafas','opini','pesan','rajin','salah','tabah','ulang',
];
...
export default function Wordle() {
  ...
  }, [currentGuess, gameOver, evaluateGuess, guesses, usedLetters, targetWord, attempt, addScore, endGame, submitScore]);
  ...
  if (!isPlaying && !gameOver) {
    return (
      <div className="flex flex-col items-center gap-6 py-10">
        ...
        <p className="text-gray-500 dark:text-slate-400 text-center max-w-md">Tebak kata 5 huruf Bahasa Indonesia dalam 6 percobaan!</p>
            ...
            {row.map((key) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    'px-2.5 py-3 rounded-md font-bold text-sm min-w-[32px] transition-colors',
                    key.length > 1 ? 'px-3 text-xs' : '',
                    keyStatus ? statusColor[keyStatus] : 'bg-gray-200 text-gray-800 hover:bg-gray-300 dark:bg-slate-700 dark:text-white dark:hover:bg-slate-600'
                  )}
                >
                  {key}
                </button>
