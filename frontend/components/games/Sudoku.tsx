  ...
  if (!isPlaying && !gameOver) {
    return (
      <div className="flex flex-col items-center gap-6 py-10">
        ...
        <p className="text-gray-500 dark:text-slate-400 text-center max-w-md">Isi grid 9x9 dengan angka 1-9!</p>
  ...
  return (
    <div className="flex flex-col items-center gap-4 py-6">
      <div className="flex items-center gap-4">
        <Timer initialSeconds={diff === 'easy' ? 600 : diff === 'medium' ? 450 : 300} onTimeUp={handleTimeUp} isRunning={isPlaying && !gameOver} />
          ...
          row.map((cell, c) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'w-9 h-9 sm:w-10 sm:h-10 flex items-center justify-center text-sm font-bold border border-gray-300 dark:border-slate-600 transition-colors',
                  ...
                  isOriginal ? 'bg-gray-100 text-gray-800 dark:bg-slate-700 dark:text-white' : 'bg-white dark:bg-slate-800',
                  isSelected && 'bg-indigo-100 dark:bg-indigo-900/30',
                  hasError && 'text-red-500 bg-red-50 dark:bg-red-900/20',
      ...
      <div className="flex gap-2">
        {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((n) => (
          <button key={n} onClick={() => handleNumberInput(n)}
            className="w-10 h-10 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-lg transition-colors"
          >{n}</button>
        ))}
      </div>
      ...
      {gameOver && (
        <div className="text-center space-y-2">
          <p className={cn('text-lg font-bold', errors.size === 0 ? 'text-emerald-600' : 'text-red-500')}>
            {errors.size === 0 && current.flat().every((c, i) => c === solution.flat()[i]) ? 'Selamat! Puzzle Solved!' : 'Game Over!'}
          </p>
          {result && (
