...
test.describe("Leaderboard page", () => {
    ...
    test("page loads and shows the Papan Peringkat heading", async ({
        page,
    }) => {
        // t('leaderboard.title') = 'Papan Peringkat'
        await expect(
            page.getByRole("heading", { name: /papan peringkat/i }),
        ).toBeVisible();
    });
    ...
    test("has All-Time and Mingguan period toggle buttons", async ({ page }) => {
        ...
        await expect(
            page.getByRole("button", { name: /mingguan/i }),
        ).toBeVisible();
    });
    ...
    test("has Global and Per Game tab buttons", async ({ page }) => {
        ...
        const tabButtons = page.locator(
            'button:has-text("Global"), button:has-text("Per Game")',
        );
        await expect(tabButtons.first()).toBeVisible();
    });

    test("shows leaderboard table or empty state after load", async ({
        page,
    }) => {
        ...
        const hasTable =
            (await page.locator("table, [class*='LeaderboardTable']").count()) > 0;
    ...
    test("empty state shows trophy icon text when no scores", async ({
        page,
    }) => {
        ...
        const emptyText = page.getByText(
            /belum ada skor|jadilah yang pertama/i,
        );
