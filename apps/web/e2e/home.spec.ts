...
test.describe("Home page — smoke tests", () => {
    ...
    test("navbar contains Login and Register links when logged out", async ({
        page,
    }) => {
        await expect(
            page.getByRole("link", { name: /masuk|login/i }).first(),
        ).toBeVisible();
        await expect(
            page.getByRole("link", { name: /daftar|register/i }).first(),
        ).toBeVisible();
    });

    test("leaderboard CTA button is present", async ({ page }) => {
        await expect(
            page.getByRole("link", { name: /lihat ranking/i }),
        ).toBeVisible();
    });
});
