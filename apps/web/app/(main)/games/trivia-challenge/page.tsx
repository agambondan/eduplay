        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              onKeyDown={(e) =>
                e.key === 'Enter' && !mutation.isPending && username && mutation.mutate()
              }
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-amber-500 to-orange-600 px-6 py-3.5 font-semibold text-white shadow-lg transition-all hover:scale-[1.02] active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50"
          >
            {mutation.isPending ? (
...
function ChallengeListScreen({
  ...
  const incoming = challenges.filter(
    (c) => c.opponent_score === null && c.challenger_score !== null
  );
        ...
        {incoming.length > 0 && (
          <section>
            ...
            <div className="space-y-2">
              {incoming.map((c) => (
                <ChallengeCard key={c.id} challenge={c} onClick={() => onSelect(c.id)} />
              ))}
            </div>
          </section>
        )}
...
function ChallengeCard({
  ...
  return (
    <button
      onClick={onClick}
      className="flex w-full items-center justify-between rounded-xl border border-gray-100 bg-white p-4 text-left shadow-sm transition-all hover:border-amber-300 hover:shadow-md dark:border-slate-700 dark:bg-slate-800"
    >
        ...
        <div className="space-y-6 text-center">
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-800">
            ...
            <div className="mt-4 flex justify-center gap-6 text-sm">
              <div>
                <span className="font-bold text-green-600">{correctCount}</span> benar
              </div>
              <div>
              ...
              <div>
                <span className="font-bold text-amber-600">
                  {answers.filter((a) => a.timeTaken <= BONUS_TIME_THRESHOLD && a.correct).length}
                </span>{' '}
                bonus kecepatan
              </div>
            </div>
      ...
      <div className="w-full space-y-6">
        <div className="flex items-center justify-between text-sm">
          ...
          <span className="flex items-center gap-1 font-medium text-gray-700 dark:text-slate-300">
            <Zap className="h-4 w-4 text-amber-500" /> {answers.reduce((s, a) => s + a.points, 0)}{' '}
            pts
          </span>
        </div>
        ...
        <div className="flex items-center justify-center gap-2 text-3xl font-bold">
          ...
          <span className={timeLeft <= 3 ? 'text-red-500' : 'text-gray-900 dark:text-white'}>
            {timeLeft}
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 gap-3">
          {options.map((opt: string, i: number) => {
            ...
            const showWrong =
              guessed && isSelected && !(opt === q.answer || opt === q.correctAnswer);

            let btnClass =
              'border-gray-200 bg-white hover:border-amber-400 hover:bg-amber-50 dark:border-slate-600 dark:bg-slate-800 dark:hover:bg-amber-950';
            if (isSelected && !guessed)
              btnClass = 'border-amber-500 bg-amber-50 dark:bg-amber-950 dark:border-amber-400';
            if (showCorrect)
              btnClass = 'border-green-500 bg-green-50 dark:bg-green-950 dark:border-green-400';
            if (showWrong)
              btnClass = 'border-red-500 bg-red-50 dark:bg-red-950 dark:border-red-400';
      ...
      <div className="space-y-6 text-center">
        <div
          className={`mx-auto flex h-24 w-24 items-center justify-center rounded-full bg-gradient-to-br shadow-lg ${
            isWin
              ? 'from-amber-400 to-yellow-500 shadow-yellow-200'
              : isDraw
                ? 'from-blue-400 to-indigo-500 shadow-blue-200'
