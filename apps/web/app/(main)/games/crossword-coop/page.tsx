...
import { ArrowLeft, Loader2, Medal, Trophy, RotateCcw, Home, Zap, Users } from 'lucide-react'
...
interface GameOverInfo {
  winner_id: string
  xp_earned: number
  results: { player_id: string; username: string; score: number; correct: number; is_winner: boolean }[]
}
  ...
  return (
    <GameContainer maxWidth="max-w-lg">
      <div className="space-y-6 pt-12 text-center">
        ...
        <p className="text-gray-500 dark:text-slate-400">Isi TTS bersama-sama! Semua pemain kerja sama selesaikan puzzle. Siapa MVP?</p>
        <button onClick={() => botMutation.mutate()} disabled={botMutation.isPending}
          ...
          {botMutation.isPending ? <Loader2 className="mx-auto h-5 w-5 animate-spin" /> : 'Mulai / Gabung Tim'}
        </button>
      </div>
    </GameContainer>
  )
...
function CoopScreen({ token, roomID, userID, username, onResult }: {
  token: string; roomID: string; userID: string; username: string; onResult: (info: GameOverInfo) => void
}) {
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent) => {
    ...
    if (e.key === 'Tab' || e.key === 'Enter') {
      ...
      if (nr < puzzle.gridSize && nc < puzzle.gridSize && puzzle.grid[nr][nc] !== '#') setSelected([nr, nc])
      return
    }
      ...
      <div className="w-full space-y-4 py-4" onKeyDown={handleKeyDown}>
        <div className="flex items-center justify-between text-sm">
          ...
          <div className="flex items-center gap-2">
            <div className="h-2 w-20 rounded-full bg-gray-200"><div className="h-2 rounded-full bg-teal-500 transition-all" style={{ width: `${progressPct}%` }} /></div>
        ...
        <div className="flex flex-wrap gap-2 text-xs">
          {players.map((name, i) => (
            <span key={i} className="rounded-full bg-teal-100 px-2.5 py-1 font-medium text-teal-700 dark:bg-teal-900 dark:text-teal-300">
              {name} {playerCounts[name] ? `(${playerCounts[name]})` : ''}
            </span>
          ))}
        </div>

        <div className="mx-auto grid gap-0 border-2 border-gray-800 rounded overflow-hidden"
          style={{ gridTemplateColumns: `repeat(${puzzle.gridSize}, minmax(0, 1fr))`, maxWidth: `${puzzle.gridSize * 44}px` }}
        >
          {puzzle.grid.map((row, r) =>
            row.map((cell, c) => {
              if (cell === '#') return <div key={`${r}-${c}`} className="bg-gray-900 aspect-square" />
              const key = `${r}-${c}`
              ...
              return (
                <button key={key} onClick={() => handleCellClick(r, c)}
                  className={`relative aspect-square flex items-center justify-center text-lg font-bold outline-none transition-colors border border-gray-300 dark:border-slate-600
        ...
        <div className="flex gap-4 text-sm">
          <div className="flex-1 space-y-1">
            ...
            {puzzle.clues.filter((cl) => cl.d === 'across').map((cl) => (
              <p key={`a${cl.n}`} className="text-xs text-gray-600 dark:text-slate-400"><span className="font-bold">{cl.n}.</span> {cl.c}</p>
            ))}
          </div>
          <div className="flex-1 space-y-1">
            <p className="font-bold text-emerald-600">Menurun</p>
            {puzzle.clues.filter((cl) => cl.d === 'down').map((cl) => (
              <p key={`d${cl.n}`} className="text-xs text-gray-600 dark:text-slate-400"><span className="font-bold">{cl.n}.</span> {cl.c}</p>
            ))}
          </div>
        </div>
      </div>
    ...
    <GameContainer maxWidth="max-w-lg">
      <div className="space-y-6 py-8 text-center">
        <div className={`mx-auto flex h-24 w-24 items-center justify-center rounded-full bg-gradient-to-br shadow-lg ${
          isMvp ? 'from-amber-400 to-orange-500 shadow-amber-200' : 'from-teal-400 to-emerald-500 shadow-teal-200'
        }`}>
        ...
        <div className="space-y-2">
          {sorted.map((r, i) => (
            <div key={r.player_id} className={`rounded-xl border p-4 text-left ${
              i === 0 ? 'border-amber-200 bg-amber-50 dark:bg-amber-950' :
              r.player_id === userID ? 'border-teal-200 bg-teal-50 dark:bg-teal-950' :
              'border-gray-100 bg-white dark:border-slate-700 dark:bg-slate-800'
            }`}>
              <div className="flex items-center gap-2">
                {i === 0 && <span className="text-lg">👑</span>}
                <p className="font-bold text-gray-900 dark:text-white">{r.username} {r.player_id === userID ? '(Kamu)' : ''}</p>
              </div>
              <p className="text-sm text-gray-500">{r.score} sel • {Math.round(r.score / Math.max(result.results?.reduce((a, b) => a + b.score, 0) || 1, 1) * 100)}% kontribusi</p>
            </div>
          ))}
        </div>
        ...
        <div className="flex gap-3">
          <button onClick={onReplay} className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-teal-500 px-5 py-3 font-bold text-white shadow-lg hover:bg-teal-600">
            <RotateCcw className="h-5 w-5" /> Main Lagi
          </button>
          <button onClick={() => router.push('/games')} className="flex flex-1 items-center justify-center gap-2 rounded-xl border-2 border-gray-200 px-5 py-3 font-bold text-gray-600 hover:bg-gray-50 dark:border-slate-600 dark:text-slate-300">
