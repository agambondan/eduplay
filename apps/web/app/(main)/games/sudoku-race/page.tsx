...
import { ArrowLeft, Loader2, Medal, Trophy, RotateCcw, Home, Zap, Flag, Clock } from 'lucide-react'
  ...
  return (
    <>
      ...
      {screen === 'playing' && matchInfo && (
        <RaceScreen token={token!} roomID={matchInfo.room_id} userID={user?.id || ''} onResult={(info) => { setGameResult(info); setScreen('result') }} />
      )}
...
function RaceScreen({ token, roomID, userID, onResult }: { token: string; roomID: string; userID: string; onResult: (info: GameOverInfo) => void }) {
      ...
      <div className="w-full space-y-4 py-4">
        <div className="flex items-center justify-between gap-4 text-sm">
          <div className="flex items-center gap-2">
            ...
            <span className={`font-mono font-bold ${timeLeft < 60 ? 'text-red-500' : 'text-gray-700 dark:text-slate-300'}`}>
          ...
          <div className="flex flex-1 items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="mt-0.5 h-2 rounded-full bg-gray-200 dark:bg-slate-700">
                <div className="h-2 rounded-full bg-emerald-500 transition-all" style={{ width: `${opponentProgress}%` }} />
              </div>
        ...
        <div className="mx-auto grid w-fit grid-cols-9 gap-0 border-2 border-gray-800 rounded overflow-hidden">
          {userGrid.map((row, r) =>
            row.map((cell, c) => {
    ...
    <GameContainer maxWidth="max-w-lg">
      <div className="space-y-6 py-8 text-center">
        <div className={`mx-auto flex h-24 w-24 items-center justify-center rounded-full bg-gradient-to-br shadow-lg ${
          isWin ? 'from-purple-400 to-indigo-500 shadow-purple-200' : 'from-gray-300 to-gray-400 shadow-gray-200 dark:from-slate-600 dark:to-slate-700'
        }`}>
          {isWin ? (
        ...
        <h1 className={`text-3xl font-extrabold ${isWin ? 'text-purple-600 dark:text-purple-400' : 'text-gray-500'}`}>
