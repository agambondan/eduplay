  ...
  return (
    <GameContainer maxWidth="max-w-lg">
      <div className="space-y-6 pt-12 text-center">
        ...
        <p className="text-gray-500 dark:text-slate-400">Game matematika estafet tim! 2-4 player saling bergantian jawab soal.</p>
        <button onClick={() => botMutation.mutate()} disabled={botMutation.isPending}
          ...
          {botMutation.isPending ? <Loader2 className="mx-auto h-5 w-5 animate-spin" /> : 'Mulai / Cari Tim'}
        </button>
      </div>
    </GameContainer>
  )
...
function RelayScreen({ token, roomID, userID, username, onResult }: {
  ...
  const [lastResult, setLastResult] = useState<{ isCorrect: boolean; scoreDelta: number } | null>(null)
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (msg.type === 'opponent_progress') {
          setCorrect((prev) => ({ ...prev, [msg.payload.player_id]: msg.payload.questions_answered }))
        }
      ...
      <div className="w-full space-y-4 py-4">
        <div className="flex items-center justify-between text-sm">
          <div className="flex items-center gap-2">
            <span className={`font-mono font-bold ${timeLeft <= 3 ? 'text-red-500' : 'text-gray-700 dark:text-slate-300'}`}>
              ⏱ {timeLeft}s
            </span>
          </div>
          ...
          <span className="text-xs text-gray-500">{questionNum}/{totalQ}</span>
        </div>
        ...
        {lastResult && (
          <div className={`rounded-xl p-3 text-center text-sm font-bold ${
            ...
            {lastResult.isCorrect ? `Benar! +${lastResult.scoreDelta} poin` : `Salah ${lastResult.scoreDelta}`}
          </div>
        )}
    ...
    <GameContainer maxWidth="max-w-lg">
      <div className="space-y-6 py-8 text-center">
        <div className={`mx-auto flex h-24 w-24 items-center justify-center rounded-full bg-gradient-to-br shadow-lg ${
          isWin ? 'from-orange-400 to-red-500 shadow-orange-200' : 'from-gray-300 to-gray-400 shadow-gray-200'
        }`}>
        ...
        <div className="space-y-2">
          {result.results?.map((r) => (
            <div key={r.player_id} className={`rounded-xl border p-4 text-left ${
              r.player_id === userID ? 'border-orange-200 bg-orange-50 dark:bg-orange-950' : 'border-gray-100 bg-white dark:border-slate-700 dark:bg-slate-800'
            }`}>
              <p className="font-bold text-gray-900 dark:text-white">{r.username} {r.player_id === userID ? '(Kamu)' : ''}</p>
              <p className="text-sm text-gray-500">{r.score} poin • {r.correct} benar</p>
            </div>
          ))}
        </div>
