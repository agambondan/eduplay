...
import { ArrowLeft, Loader2, RotateCcw, Home, Zap, Bot, User, Globe } from 'lucide-react'
...
export default function ChessPage() {
  ...
  const createMutation = useMutation({
    mutationFn: (data: { vs_bot: boolean; bot_difficulty?: Difficulty; player_color?: 'white' | 'black' }) =>
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (msg.type === 'game_over') {
          setMatch((prev) => prev ? { ...prev, status: 'finished', winner_id: msg.payload.winner_id } : prev)
  ...
  const handleSquareClick = useCallback((square: string) => {
    ...
    if (gameMode === 'ws') {
      ...
      if (pieceAtTarget && pieceAtTarget.color === chessInstance.get(selectedSquare as Square)?.color) {
      ...
      try {
        ...
        wsRef.current?.send(JSON.stringify({ type: 'chess_move', payload: { room_id: matchId, move: san } }))
    ...
    const myTurn = isBotGame
      ? (isPlayerWhite ? chessInstance.turn() === 'w' : chessInstance.turn() === 'b')
      : true
    if (!myTurn) return
    ...
    try {
      ...
      if (isBotGame) {
        ...
        setTimeout(() => {
          ...
          const bestMove = findBestMove(aiCopy, (match.bot_difficulty || 'medium') as Difficulty)
          if (bestMove) {
  ...
  const getSquareColor = (r: number, c: number) => (r + c) % 2 === 0 ? 'bg-amber-100' : 'bg-amber-800'
  ...
  function renderBoard() {
    ...
    const legalMoves = selectedSquare ? chessInstance.moves({ square: selectedSquare as Square, verbose: true }) : []

    return (
      <div className="mx-auto w-full max-w-[min(480px,calc(100vw-32px))] select-none" style={{ touchAction: 'none' }}>
        <div className="grid grid-cols-8 gap-0 border-2 border-gray-800 rounded overflow-hidden shadow-lg">
        {board.map((row, r) =>
          row.map((piece, c) => {
            ...
            return (
              <button
                ...
                className={`relative flex aspect-square items-center justify-center text-[clamp(1.25rem,5vw,2.25rem)] transition-colors
                  ...
                  ${isLastMove && !isSelected ? 'ring-1 ring-yellow-300 ring-inset' : ''}
  ...
  function renderGameStatus() {
    ...
    return (
      <p className="text-sm text-gray-500">
        ...
        {match?.is_vs_bot && (
          <span className="ml-2">
            (Kamu: {match.player_color === 'white' ? 'Putih' : 'Hitam'})
          </span>
        )}
      </p>
    )
      ...
      <>
        ...
        <GameContainer maxWidth="max-w-lg">
          <div className="space-y-6 pt-12 text-center">
            ...
            <p className="text-gray-500 dark:text-slate-400">Catur klasik — Quick Match atau vs Bot!</p>
            {error && <p className="text-sm text-red-500">{error}</p>}
            <div className="space-y-3">
              <button onClick={() => quickMatchMutation.mutate()}
                ...
                {quickMatchMutation.isPending ? <Loader2 className="h-5 w-5 animate-spin" /> : <Globe className="h-5 w-5" />}
                Cari Lawan Online
              </button>
              <div className="relative py-2">
                ...
                <div className="relative flex justify-center"><span className="bg-white px-3 text-xs text-gray-400 dark:bg-slate-900">atau</span></div>
              </div>
              <p className="text-sm font-semibold text-gray-600 dark:text-slate-300">Vs Bot</p>
              <div className="flex justify-center gap-3">
                {(['easy', 'medium', 'hard'] as const).map((d) => (
                  <button key={d} onClick={() => createMutation.mutate({ vs_bot: true, bot_difficulty: d, player_color: 'white' })}
                    ...
                    <Bot className="h-4 w-4" /> {d === 'easy' ? 'Mudah' : d === 'medium' ? 'Sedang' : 'Sulit'}
                  </button>
                ))}
              </div>
              <button onClick={() => createMutation.mutate({ vs_bot: true, bot_difficulty: 'medium', player_color: 'black' })}
                disabled={createMutation.isPending}
                className="flex items-center justify-center gap-2 w-full rounded-xl border-2 border-stone-400 px-6 py-3 font-semibold text-stone-700 transition-all hover:bg-stone-50 dark:text-stone-300">
        ...
        <GameContainer maxWidth="max-w-lg">
          <div className="space-y-6 py-8 text-center">
            <div className={`mx-auto flex h-24 w-24 items-center justify-center rounded-full bg-gradient-to-br shadow-lg ${
              isWin ? 'from-emerald-400 to-teal-500 shadow-emerald-200' : isDraw ? 'from-yellow-300 to-yellow-500 shadow-yellow-200' : 'from-gray-300 to-gray-400 shadow-gray-200'
            }`}>
              <span className="text-5xl">{isWin ? PIECE_UNICODE['wK'] : isDraw ? '\u2694' : PIECE_UNICODE['bK']}</span>
            </div>
            <h1 className={`text-3xl font-extrabold ${isWin ? 'text-emerald-600' : isDraw ? 'text-yellow-600' : 'text-gray-500'}`}>
              {isWin ? 'Kamu Menang!' : isDraw ? 'Seri!' : 'Kamu Kalah'}
            </h1>
            {match?.win_reason && (
              <p className="text-sm text-gray-500 capitalize">{match.win_reason === 'resign' ? 'Lawan resign' : match.win_reason}</p>
            )}
      ...
      <GameContainer maxWidth="max-w-xl">
        <div className="w-full space-y-4 py-4">
          ...
          <div className="flex gap-2">
            <button onClick={() => {
              if (gameMode === 'ws') {
                wsRef.current?.send(JSON.stringify({ type: 'leave_room', payload: { room_id: matchId } }))
                setScreen('result')
              } else {
            ...
            <div className="max-h-24 overflow-y-auto rounded-xl border border-gray-200 bg-gray-50 p-3 dark:border-slate-700 dark:bg-slate-800">
              <div className="flex flex-wrap gap-1">
                {moveHistory.map((mv, i) => (
                  <span key={i} className="rounded bg-white px-2 py-0.5 text-xs font-mono shadow-sm dark:bg-slate-700 dark:text-slate-300">
