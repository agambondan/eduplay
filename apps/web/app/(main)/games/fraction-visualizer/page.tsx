  ...
  return (
    <div className="flex h-10 w-full overflow-hidden rounded-lg border border-gray-200 dark:border-slate-600">
      {Array.from({ length: denominator }, (_, i) => (
        <div
          key={i}
          className={`flex-1 border-r border-white/50 transition-colors last:border-r-0 ${i < numerator ? color : 'bg-gray-100 dark:bg-slate-700'}`}
        />
      ))}
    </div>
  );
...
export default function FractionVisualizerPage() {
  ...
  const { isPlaying, score, startGame, endGame, addScore, submitScore } = useGame(
    'fraction-visualizer',
    'Fraction Visualizer',
    'math'
  );
