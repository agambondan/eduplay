...
export default function QuizShowdownPage() {
        ...
        <div className="space-y-6 pt-4">
          ...
          <div className="text-center">
            ...
            <p className="mt-2 text-sm text-gray-500">
              Real-time quiz battle 2-4 player dengan room yang bisa disesuaikan.
            </p>
          </div>
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center">
              <span className="bg-white px-3 text-xs text-gray-400 dark:bg-gray-950">atau</span>
            </div>
          </div>
          <button
            onClick={() => setScreen('join')}
            className="flex w-full items-center justify-center gap-3 rounded-xl border-2 px-6 py-4 font-semibold text-gray-700 hover:border-pink-300"
          >
      ...
      <GameContainer maxWidth="max-w-lg">
        <div className="space-y-6 pt-4">
          ...
          <input
            ...
            placeholder="ABC123"
            className="w-full rounded-xl border p-4 text-center text-2xl font-bold uppercase tracking-[0.35em] focus:border-pink-500 focus:outline-none dark:bg-slate-800"
            maxLength={6}
          />
          <button
            onClick={() => joinMutation.mutate()}
            disabled={roomCode.length !== 6 || joinMutation.isPending}
            className="flex w-full items-center justify-center gap-3 rounded-xl bg-gradient-to-r from-pink-500 to-rose-600 px-6 py-4 font-semibold text-white shadow-lg disabled:opacity-50"
          >
  ...
  if (screen === 'lobby' && roomInfo) {
    ...
    const canStart = Boolean(
      isHost && (roomInfo.settings.allow_bots || roomInfo.members.length >= 2)
    );
    return (
      <GameContainer maxWidth="max-w-lg">
        <div className="space-y-6 pt-4">
          <div className="flex items-center justify-between">
            ...
            <button onClick={refreshLobby} className="text-xs text-gray-400">
              Refresh
            </button>
          </div>
          <div className="text-center">
            ...
            <div className="mt-2 flex items-center justify-center gap-3">
              <span className="text-4xl font-extrabold tracking-[0.25em] text-pink-600">
                {roomInfo.room_code}
              </span>
          ...
          {!isHost && (
            <p className="text-center text-sm text-gray-400">Menunggu host memulai game...</p>
          )}
        </div>
      </GameContainer>
    );
            ...
            <div className="space-y-4">
              <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-800">
                <p className="mb-1 text-xs text-gray-400">
                  Soal {currentQ.question_number}/{currentQ.total}
                </p>
              ...
              {lastResult && (
                <div
                  className={`rounded-xl p-4 text-center text-sm font-semibold ${lastResult.is_correct ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}
                >
                  {lastResult.is_correct
                    ? `Benar! +${lastResult.score_delta} pts`
                    : `Salah ${lastResult.score_delta} pts`}
                </div>
              )}
            </div>
          )}
      ...
      <GameContainer maxWidth="max-w-lg">
        <div className="space-y-6 pt-4 text-center">
          ...
          <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white">
            {myResult?.is_winner ? 'Kamu Menang!' : 'Game Selesai!'}
          </h1>
          {myResult && <p className="text-gray-500">Skor akhir: {myResult.score} poin</p>}
          <ShareButton
            title="Quiz Showdown - EduPlay"
            text={`Aku baru saja main Quiz Showdown di EduPlay dengan skor ${myResult?.score ?? 0}. Main yuk!`}
          />
          <div className="flex gap-3">
            <button
              onClick={() => router.refresh()}
              className="flex-1 rounded-xl bg-gradient-to-r from-pink-500 to-rose-600 px-6 py-3 font-semibold text-white shadow-lg"
            >
              Main Lagi
            </button>
            <button
              onClick={() => router.push('/games')}
              className="flex-1 rounded-xl border-2 px-6 py-3 font-semibold text-gray-700"
            >
              Kembali
            </button>
          </div>
...
function SettingsPanel({
  ...
  return (
    <div
      className={`space-y-4 rounded-2xl border border-gray-200 bg-white p-5 dark:border-slate-700 dark:bg-slate-800 ${compact ? 'text-sm' : ''}`}
    >
      <h3 className="text-sm font-semibold">Pengaturan Room</h3>
      <Segment
        label="Jumlah Soal"
        options={[10, 20, 30].map((n) => ({ key: String(n), label: String(n), value: n }))}
      ...
      <Segment
        ...
        onChange={(category) => onChange({ ...settings, category })}
      />
      <Segment
        label="Difficulty"
        options={[
      ...
      <Segment
        ...
        options={[5, 10, 15, 30].map((n) => ({ key: String(n), label: `${n} dtk`, value: n }))}
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {[2, 3, 4].map((n) => (
            <button
              key={n}
              onClick={() => onChange({ ...settings, max_players: n })}
              className={`h-10 w-10 rounded-lg border-2 text-sm font-bold ${settings.max_players === n ? 'border-pink-500 bg-pink-50 text-pink-700' : 'border-gray-200 text-gray-600'}`}
            >
              {n}
            </button>
          ))}
      ...
      <label className="flex items-center justify-between">
        ...
        <input
          type="checkbox"
          checked={settings.allow_bots}
          onChange={(e) => onChange({ ...settings, allow_bots: e.target.checked })}
  ...
  return (
    <div className="rounded-2xl border p-4">
      <h3 className="mb-3 text-sm font-semibold">
        Player ({members.length}/{maxPlayers})
      </h3>
      <div className="space-y-2">
        {members.map((member) => (
          <div
            key={member.id}
            className="flex items-center justify-between rounded-xl bg-gray-50 p-3 dark:bg-slate-800"
          >
  ...
  return (
    <div className="grid grid-cols-2 gap-2">
      {members.map((member) => (
        <div
          key={member.id}
          className={`rounded-xl p-3 text-center ${member.id === userID ? 'bg-pink-50 text-pink-700' : 'bg-gray-50 text-gray-600'}`}
        >
          <p className="truncate text-xs font-semibold">
            {member.id === userID ? 'Kamu' : member.username}
          </p>
