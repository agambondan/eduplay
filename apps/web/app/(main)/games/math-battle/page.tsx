...
import { ArrowLeft, Loader2, Sparkles, Target, Trophy, Wifi, WifiOff, Zap } from 'lucide-react';
  ...
  return (
    <>
      <button
        ...
        className="absolute left-4 top-4 z-10 flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700 dark:text-slate-400"
      >
      ...
      {(screen === 'searching' || screen === 'playing') && matchInfo && (
        <SearchingScreen
          ...
          onBack={() =>
            tournamentParams ? router.push('/games/math-tournament') : setScreen('menu')
          }
        />
      )}
      {screen === 'result' && (
...
function MenuScreen({ onStart }: { onStart: (r: QuickMatchResult) => void }) {
  ...
  const botMutation = useMutation({
    mutationFn: (useRecommended: boolean) =>
      multiplayerApi.quickMatchBot('math-battle', difficulty, useRecommended),
    onSuccess: (result) =>
      onStart({
      ...
      <div className="space-y-6 pt-8">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-gray-500">
            Head-to-head real-time! Jawab soal lebih cepat dan benar dari lawan.
          </p>
        </div>
...
function SearchingScreen({
  ...
  useEffect(() => {
    if (!gameResult || !tournamentParams || tournamentParams.spectator || interstitialShown.current)
      return;
        ...
        <GameContainer maxWidth="max-w-lg">
          <div className="space-y-6 pt-8 text-center">
            ...
            <div className="flex items-center justify-center gap-8">
              <div className="text-center">
                <p className="text-3xl font-extrabold">{finalMyScore}</p>
                <p className="text-sm text-gray-500">Skormu</p>
              </div>
              <div className="text-2xl font-bold text-gray-300">:</div>
              <div className="text-center">
                <p className="text-3xl font-extrabold">{finalOpponentScore}</p>
                <p className="text-sm text-gray-500">{opponentName}</p>
              </div>
            </div>
            {myResult && (
              <p className="text-sm text-gray-500">
                {myResult.correct} benar · {myResult.wrong} salah
              </p>
            )}
            {!tournamentParams?.spectator && (
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 rounded-xl bg-indigo-600 px-6 py-3 font-semibold text-white"
              >
                Main Lagi
              </button>
              <button
