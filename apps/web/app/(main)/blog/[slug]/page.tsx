...
export default function BlogPostPage() {
  ...
  if (!post) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-20 text-center">
        ...
        <Link href="/blog" className="mt-4 inline-flex items-center gap-1 text-indigo-600 hover:text-indigo-700">
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <Link href="/blog" className="mb-6 inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700 dark:text-slate-400">
        <ArrowLeft className="h-4 w-4" /> Kembali ke Blog
      </Link>
      ...
      <article>
        ...
        <h1 className="mt-4 text-3xl font-extrabold leading-tight text-gray-900 dark:text-white">{post.title}</h1>
        {post.excerpt && <p className="mt-3 text-lg leading-relaxed text-gray-500 dark:text-slate-400">{post.excerpt}</p>}

        <div className="mt-8 prose prose-gray max-w-none dark:prose-invert" dangerouslySetInnerHTML={{ __html: post.content }} />

        {post.tags && (
          <div className="mt-10 flex flex-wrap gap-2 border-t border-gray-200 pt-6 dark:border-slate-700">
            {post.tags.split(',').map((tag) => (
              <span key={tag} className="flex items-center gap-1 rounded-lg bg-gray-100 px-3 py-1 text-sm text-gray-600 dark:bg-slate-800 dark:text-slate-300">
