...
const DEFAULT_SLOTS: Record<string, AdSlotConfig> = {
  banner: { slot: 'banner', sizes: [[728, 90], [320, 50]], network: 'direct' },
...
export function useAdManager() {
  ...
  useEffect(() => {
    ...
    if (cfg) {
      try { setNetworkOrder(JSON.parse(cfg)) } catch {}
    }
  }, [])

  const loadGPT = useCallback(() => {
    if (gptLoaded || typeof window === 'undefined') return
    if ((window as any).googletag?.apiReady) { setGptLoaded(true); return }
