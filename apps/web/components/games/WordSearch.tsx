...
export default function WordSearch() {
  const { score, isPlaying, startGame, endGame, addScore, submitScore } = useGame('word-search');
  ...
  if (!isPlaying && !gameOver) {
    return (
      <div className="flex flex-col items-center gap-6 py-10">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Word Search</h1>
        <p className="max-w-md text-center text-gray-500 dark:text-slate-400">
          Cari kata-kata tersembunyi di dalam grid!
        </p>
        <button
          onClick={handleStart}
    ...
    <div className="flex flex-col items-center gap-6 py-6">
      ...
      {gameOver && (
        <div className="space-y-2 text-center">
          <p className="text-xl font-bold text-emerald-600">Hebat! Semua kata ditemukan!</p>
          {result && <p className="text-sm text-gray-500">+{result.xp} XP earned</p>}
          <button
            onClick={handleStart}
