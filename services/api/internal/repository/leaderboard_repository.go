...
func (r *leaderboardRepository) AddScore(key string, userID string, score float64) error {
...
func (r *leaderboardRepository) GetTopN(key string, n int64) ([]redis.Z, error) {
	ctx := context.Background()
	return database.RDB.ZRevRangeWithScores(ctx, key, 0, n-1).Result()
}

func (r *leaderboardRepository) GetUserRank(key string, userID string) (int64, error) {
	ctx := context.Background()
